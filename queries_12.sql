-- @C:\Oracledatabase\CSY1026\queries_12.sql
...
FROM prizes 
WHERE achievement = 'GOAT';

-- negative condition 
SELECT achievement, prize_id, sponsor_id
FROM prizes 
WHERE NOT achievement = 'GOAT';
-- conditions for achievement that ends with T
SELECT * 
FROM prizes;
WHERE advertisement = '%T';


-- condition by order of round
SELECT round_id,round_level,stage
FROM rounds
ORDER BY round_id;

-- condition for round_levels between 1 and 500
SELECT round_id,round_level,stage
FROM rounds
WHERE round_level BETWEEN 1 AND 500
ORDER BY stage;

-- condition for POG that are greater than 5
SELECT statistic_id,game_id,duration
FROM statistics
WHERE pog > 5
ORDER BY duration;

-- Joined queries with 2 tables
